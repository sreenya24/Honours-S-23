...
def superimpose_all_inliers(ransac_lines,width:float, height:float, result):
...
def extract_multiple_lines_and_save(result, heightPatch, inputfilename:str,iterations:int, max_distance:int,min_inliers_allowed:int):
    """
    ...
    patches = height // heightPatch
    for index in range(0,patches):
        starting_points=all_black_points[index*heightPatch:(index+1)*heightPatch]
        if (len(starting_points) <= min_samples):
            print("No more points available. Terminating search for RANSAC")
            break
        inlier_points,_,model=extract_first_ransac_line(starting_points,max_distance=max_distance)
        ...
        temp = superimpose_all_inliers(results,width,height, result[heightPatch*index:heightPatch*(index+1)])
        result[heightPatch*index:heightPatch*(index+1)] = temp.copy()
        # plt.figure()
        # plt.axis('off')
        # plt.imshow(result)
        # plt.show()
    ...
    # superimposed_image=superimpose_all_inliers(results,width,height, result)
    superimposed_image = result.copy()
    #Save the results
    filename_noextension=os.path.splitext(inputfilename)[0]
    folder_script=os.path.dirname(__file__)
    file_result=os.path.join(folder_script,"./out/",("result.png"))
    io.imsave(file_result,superimposed_image)
    print("Results saved to file %s" % (file_result))


height = 20
result = cv2.imread('./images/dilated.png')
result = cv2.cvtColor(result, cv2.COLOR_BGR2RGB)
extract_multiple_lines_and_save(result, height, "./dilated.png",iterations= 30,max_distance=1, min_inliers_allowed=15)
